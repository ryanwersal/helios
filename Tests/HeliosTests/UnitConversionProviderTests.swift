...
struct UnitConversionProviderTests {
    ...
    @Test
    func `cups to liters`() async {
        let results = await provider.search(query: "5 cups in liters")
        #expect(results.count == 1)
        #expect(results[0].title.contains("1.2"))
    }

    @Test
    func `kg to lbs`() async {
        let results = await provider.search(query: "100 kg to lbs")
        #expect(results.count == 1)
        #expect(results[0].title.contains("220.462"))
    }

    @Test
    func `fahrenheit to celsius`() async {
        let results = await provider.search(query: "72 fahrenheit in celsius")
        #expect(results.count == 1)
        #expect(results[0].title.contains("22.2"))
    }

    @Test
    func `bare query defaults to 1`() async {
        let results = await provider.search(query: "cups to liters")
        #expect(results.count == 1)
    ...
    @Test
    func `decimal input`() async {
        let results = await provider.search(query: "2.5 km to miles")
        #expect(results.count == 1)
        #expect(results[0].title.contains("1.553"))
    }

    @Test
    func `case insensitivity`() async {
        #expect(provider.canHandle(query: "5 KG TO LBS"))
        let results = await provider.search(query: "5 KG TO LBS")
        #expect(results.count == 1)
    }
